...
typedef struct {
    ...
    unsigned char* bmap; /* buffer of width*height bytes; each element
                            represents the gray level of a pixel (0-255) */
} PGM_image;
...
int main(int argc, char* argv[]) {
    ...
    if (img.width != img.height) {
        fprintf(
            stderr,
            "FATAL: width (%d) and height (%d) of the input image must be "
            "equal\n",
            img.width,
            img.height
        );
