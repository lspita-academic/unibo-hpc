...
void generate(float sz, int iter, float cx, float cy, float cz, int dir) {
    ...
    if (dir != DIR_NX) generate(new_sz, iter - 1, cx + ofs, cy, cz, DIR_PX);

    if (dir != DIR_PX) generate(new_sz, iter - 1, cx - ofs, cy, cz, DIR_NX);

    if (dir != DIR_NY) generate(new_sz, iter - 1, cx, cy + ofs, cz, DIR_PY);

    if (dir != DIR_PY) generate(new_sz, iter - 1, cx, cy - ofs, cz, DIR_NY);

    if (dir != DIR_NZ) generate(new_sz, iter - 1, cx, cy, cz + ofs, DIR_PZ);

    if (dir != DIR_PZ) generate(new_sz, iter - 1, cx, cy, cz - ofs, DIR_NZ);
}

int main(int argc, char* argv[]) {
    int niter = 5;
    const int MAX_ITER =
        7; /* You really don't want more than these iterations */
